...
// need to read the image from database and use to create classlabel map
              ...
              std::string fileExtension)
{
  ...
  while((classdirp = readdir(classdp)) != NULL)
  {
    ...
    if(pos != std::string::npos)
    {
      ...
      //To create a vector of all unique labels
      double clsFromVector= clsLabVector(cls_lab[0], classMapString);

     if(clsFromVector == 0)
     {
       classMapString.push_back(cls_lab[0]);
       double curIndece = classMapString.size();

       classMapStToDbl.push_back(std::pair < string, double> (cls_lab[0],curIndece));
       modelFiles.push_back(std::pair<double, std::string >(curIndece, path + "/" + filename));
     }
     else
     {
        modelFiles.push_back(std::pair<double, std::string >(clsFromVector, path + "/" + filename));
     }
  ...
  std::cout<<"classLabel:"<<endl;
  for(auto const & p : classMapString)
   {
      std::cout<<p<<std::endl;
  }
...

{
  ...
  desc.add_options()
      ...
      ("storageInput,i", po::value<std::string>(& storageInput)->default_value("Kit25Db"),
       "enter input storage folder name")
      ("datasets,d", po::value<std::string>(&dataset_name)->default_value("DbImg"),
       "choose the dataset: [DbImg]")
  ...
  //Extract the feat descriptors
   std::vector<std::pair<double, std::vector<float> > > descriptors_all;
    ...
    getFilesDB(db_image, model_files_all, objectToClassLabelMap, "_.png");
